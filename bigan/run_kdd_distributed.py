...
def train_and_test(nb_epochs, weight, method, degree, random_seed, id):
    ...
    with tf.variable_scope('discriminator_model'):
        ...
        x_gen = tf.placeholder(tf.float32, shape=(None, 121), name="x_gen")
    ...
    with tf.variable_scope('generator_model'):
        reconstruct_ema = gen(z_gen_ema, is_training=is_training_pl,
                              getter=get_getter(gen_ema), reuse=tf.AUTO_REUSE)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        while epoch < nb_epochs:
            ...
            for t in range(nr_batches_train):
                ...
                _, ld, lg, g_, sm = sess.run([train_dis_op,
                                      loss_discriminator,
                                      loss_generator,
                                      g,
                                      sum_op_dis],
            ...
            if epoch % 10 == 0:
                time.sleep(2)
                if id == 1:
                    saver2.restore(sess, 'distributed/2/distributed_{}.ckpt'.format(epoch))
                    saver3.restore(sess, 'distributed/2/distributed_{}.ckpt'.format(epoch))
                elif id == 2:
                    saver2.restore(sess, 'distributed/1/distributed_{}.ckpt'.format(epoch))
                    saver3.restore(sess, 'distributed/1/distributed_{}.ckpt'.format(epoch))
