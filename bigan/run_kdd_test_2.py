...
import data.kdd_normal as data_normal
...
def averageModels(model1, model2, output, w1, w2):
    ...
    for name in var_values:
        ...
        space = sys.getsizeof(var_values[name])
        print(name, ":", var_values[name].shape, ":", var_dtypes[name], "space:", space)
        temp += space
    # a = var_values['discriminator_model/discriminator/x_layer_1/fc/bias']
    # b = var_values['discriminator_model/discriminator/x_layer_1/fc/kernel']
    # print(a)
    ...
    print(sys.getsizeof(tf_vars))
    print(sys.getsizeof(placeholders))
    print(sys.getsizeof(assign_ops))
    print(sys.getsizeof(tf.all_variables()))
    exit()
...
def test(nb_epochs, weight, method, degree, random_seed):
    ...
    # testx = data1._get_dataset()['x_test']
    # testy = data1._get_dataset()['y_test']
    # testl = data1.get_label()
    ...
    with tf.Session() as sess:
        ...
        for i in range(600):
            saver1.restore(sess, 'central/central_{}.ckpt'.format((i+1)*5))
            saver2.restore(sess, 'distributed/3/distributed_{}.ckpt'.format((i+1)*5))
            saver3.restore(sess, 'distributed/3/distributed_{}.ckpt'.format((i+1)*5))
            ...
            # saver.restore(sess, 'model/model_1_{}.ckpt'.format(25))
            ...
            # print(per)
            # # 1000            per =
            # temp = list(zip(scores, testy))
            # score_ = pd.DataFrame(columns=['score', 'label'], data=temp)
            # score_.to_csv('score.csv', index=None)
            # normal = [0 for x in range(10000)]
            # abnormal = [0 for x in range(10000)]
            # for i in range(50000):
            #     if scores[i]<100000:
            #         j = int(scores[i]/10)
            #         normal[j] = normal[j]+1
            #     if scores[i+50000]<100000:
            #         k = int(scores[i+50000]/10)
            #         abnormal[k] = abnormal[k]+1
            # temp2 = list(zip(abnormal, normal))
            # distribution = pd.DataFrame(columns=['normal', 'abnormal'], data=temp2)
            # distribution.to_csv('distribution.csv', index=None)
            # flag = 0
            # t = 0
            # for per in range(0, 100000):
            #     y_pred = scores.copy()
            ...
            #
            #     normal = y_pred[inds]
            #     abnormal = y_pred[inds_comp]
            #
            #     w0 = len(normal)/len(scores)
            #     w1 = len(abnormal)/len(scores)
            #     u0 = np.mean(normal)
            #     u1 = np.mean(abnormal)
            #     g = w0*w1*(u0-u1)*(u0-u1)
            #     if g > flag:
            ...
            #
            #     if accuracy+precision+recall+f1 > total_temp and recall>precision and recall>f1:
            #         total_temp = accuracy+precision+recall+f1
            #         acc_temp = accuracy
            #         prec_temp = precision
            #         rec_temp = recall
            #         f1_temp = f1
            ...
            #     % ((i+1)*5, acc_temp, prec_temp, rec_temp, f1_temp, auc))
            print(
                "epoch%d:Testing : Acc = %.4f | Prec = %.4f | Rec = %.4f | F1 = %.4f | Auc = %.4f"
                % ((i+1)*5, accuracy, precision, recall, f1, auc))
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        while epoch < nb_epochs:
            ...
            if epoch % 5 == 0:
                ...
                save_path = saver.save(sess, 'model0/model_1_{}.ckpt'.format(epoch))
...
def run(nb_epochs, weight, method, degree, label, random_seed=42):
    ...
    with tf.Graph().as_default():
        ...
        # averageModels("distributed/1/distributed_1.ckpt", "model_2.ckpt", 'output.csv', 0.3, 0.7)
        # # x_train, y_train = data.get_train(20)
