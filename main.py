...
def run(args):
    ...
    if args.example and args.dataset and args.split:
        try:
            if args.id == 0:
                mod_name = "{}.{}_{}_{}".format(args.example, args.split, args.dataset, "central_2")
                logger.info("Running script at {}".format(mod_name))

                mod = importlib.import_module(mod_name)
                mod.run(args.nb_epochs, args.w, args.m, args.d, args.label, args.rd)
            elif args.id == -1:
                mod_name = "{}.{}_{}_{}".format(args.example, args.split, args.dataset, "test")
                logger.info("Running script at {}".format(mod_name))
                ...
                mod_name = "{}.{}_{}_{}".format(args.example, args.split, args.dataset, "test_2")
                logger.info("Running script at {}".format(mod_name))

                mod = importlib.import_module(mod_name)
                mod.run(args.nb_epochs, args.w, args.m, args.d, args.label, args.rd)
            else:
                mod_name = "{}.{}_{}_{}".format(args.example, args.split, args.dataset, "distributed_2")
                logger.info("Running script at {}".format(mod_name))

                mod = importlib.import_module(mod_name)
                mod.run(args.nb_epochs, args.w, args.m, args.d, args.label, args.rd, args.id)
...
if __name__ == "__main__":
    ...
    parser.add_argument('dataset', nargs="?", choices=['mnist', 'kdd', 'unsw'], help='the name of the dataset you want to run the experiments on')
    ...
    parser.add_argument('--id', nargs="?", default=-1, type=int, help='the ID of SDN device, 0 represents central SDN, others represents distributed SDN')
